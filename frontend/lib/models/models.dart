...
// ── NEW: Portfolio ────────────────────────────────────────────────────────────
class HoldingItem {
  final int id; final String ticker; final double shares,costBasisPerShare,currentPrice,marketValue,costBasisTotal,unrealizedPnl,pnlPct,actualWeight,targetWeight,weightDiff; final String? dateBought,notes,priceUpdatedAt; final bool priceStale;
  const HoldingItem({required this.id,required this.ticker,required this.shares,required this.costBasisPerShare,required this.currentPrice,required this.marketValue,required this.costBasisTotal,required this.unrealizedPnl,required this.pnlPct,required this.actualWeight,required this.targetWeight,required this.weightDiff,this.dateBought,this.notes,this.priceUpdatedAt,this.priceStale=false});
  factory HoldingItem.fromJson(Map<String,dynamic> j) => HoldingItem(id:j['id']??0,ticker:j['ticker']??'',shares:(j['shares']as num).toDouble(),costBasisPerShare:(j['cost_basis_per_share']as num).toDouble(),currentPrice:(j['current_price']as num).toDouble(),marketValue:(j['market_value']as num).toDouble(),costBasisTotal:(j['cost_basis_total']as num).toDouble(),unrealizedPnl:(j['unrealized_pnl']as num).toDouble(),pnlPct:(j['pnl_pct']as num).toDouble(),actualWeight:(j['actual_weight']as num).toDouble(),targetWeight:(j['target_weight']as num).toDouble(),weightDiff:(j['weight_diff']as num).toDouble(),dateBought:j['date_bought'],notes:j['notes'],priceUpdatedAt:j['price_updated_at'],priceStale:j['price_stale']??false);
}
